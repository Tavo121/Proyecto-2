...
def win():
    ...
    def position():
        ...
        Nombre = e_jugador.get() + '---' + str(pnts) + ',' + '\n'
        Pos = 1
        Comp = Arch.readline()
        while Comp != "":
            if Comp == Nombre:
                Arch.close()
                return 'Quedó en la posición: ' + str(Pos)
            Comp = Arch.readline()
            Pos += 1
        Arch.close()
        return 'Su posicion es menor a 10'
    save_highscore()

    #rewrite(quickS_list_maker())
    Texto = position()
    Frame.create_text(250, 80, text = 'YOU WON!', font = ('OCR A Extended', 20), fill = 'white')
    Frame.create_text(250, 130, text =Texto, font = fuente, fill = 'white')
    Frame.create_text(250, 180, text = 'Su puntaje es: ' + str(pnts), font = fuente, fill = 'white')
    def waiting():
        sleep(0.5)
        rewrite(quickS_list_maker())
    waiting()
...
def save_highscore():
    ...
    ***************************************************************************"""
    ...
    def compare_names(Nombre, i):
        while Nombre[i] != "-":
            i += 1
        return Nombre[0:i]

    def compare_ptn(Puntos, Names, vueltas, i,j): #compara el puntaje obtenido con los puntajes guardados
        nonlocal Score_Usuario
        if vueltas == 10: #condicion de finalizacion
            return 'Su posición es menor a 10'
            ...
            Comparacion = Names[i][j:-2] #corte para tomar solo los puntajes del string
            if int(Comparacion) <= Puntos: #comparacion de puntajes
                Names[i] = Score_Usuario + '\n'
                Arch.seek(0)
                return rewrite(Names)
            else:
                return compare_ptn(Puntos, Names, vueltas +1, i+1, 0) #llamada recursiva
        elif compare_names(Names[i], 0) == e_jugador.get(): #comparacion de nombres
            for letra in Names[i]:
            ...
            return compare_ptn(Puntos, Names, vueltas, i, j+1) #llamada recursiva

    def compare(Puntaje, Registros, Names, Names2): #Busca si hay mas de 10 puntajes registrados
        global pnts
        if Registros == 10: #hay 10 puntajes registrados
            compare_ptn(pnts, Names2, 0, 0, 0)
        elif Names == []: #no hay 10 puntajes registrados
    ...
    def create_list(Names, Result): #crea una lista con los puntajes y nombres
        nonlocal Score_Usuario
        if Names == "":
...
#------------------------------------------------------------------------------------------------------
def insertS_list_maker(): #crea una lista con los nombres y sus puntajes y luego llama a la funcion insertion_sort_names con la lista creada
    Arch = open('Highscore.txt', 'r')
    ...
    Lista_nombres = create_list(Arch.readline(), [])
    Arch.close()
    return insertion_sort_names(Lista_nombres)
...
def insertion_sort_names(Lista):
    ...
    ***************************************************************************
    ...
    Entradas: Lista de puntajes
    Restricciones: Nombres deben ser tipo string y seguir el formato de guardado de puntajes
    Salidas: Puntajes ordenados por orden alfabetico

    ***************************************************************************"""
    return IS_names_aux(Lista, 1, len(Lista))

def IS_names_aux(Nombres, i, Cant): # ordena los puntajes por orden alfabetico
    if i == Cant:
        return Nombres
    Aux = Nombres[i] #variable de comparacion
    j = lista_ordenada(Nombres, i, Aux)
    Nombres[j] = Aux
    return IS_names_aux(Nombres, i + 1, Cant)

def lista_ordenada(Lista, i, Aux): #ordena la lista de puntajes
...
def puntaje():
    ...
    ***************************************************************************"""
    ...
    def cerrar_puntajes(): #cierre de la ventana
        ventana.deiconify()
    ...
    def update_score(event): #Actualiza los puntajes de acuerdo al ordenamiento
        Arch = open('Highscore.txt', 'r')
        def create_text(Names, Result):
            if Names == "":
                return Result
            else:
                Result = Result + Names
                Names = Arch.readline()
                return create_text(Names, Result)

        Texto = create_text(Arch.readline(), '')
        Arch.close()
        Nombres = Label(vent_puntajes,  text = Texto, font = ('OCR A Extended', 20), fg = 'black', bg = '#10304a')
        Nombres.place(x=150, y = 100)
        Nombres.config(text = Texto)

    #Botones
    BQS = Button(vent_puntajes, text='By points', font=('OCR A Extended', 10), width=9, height=1, command=lambda:rewrite(quickS_list_maker()))
    BQS.place(x=5, y=20)
    BQS.bind('<Button-1>', update_score)

    BIS = Button(vent_puntajes, text='By name', font=('OCR A Extended', 10), width=9, height=1, command=lambda:rewrite(insertS_list_maker()))
    BIS.place(x=412, y=20)
    BIS.bind('<Button-1>', update_score)
